...
Deno.serve(async (req) => {
  try {
    ...
    doc.setDrawColor(203, 213, 225);
    doc.line(margin, yPos + 20, margin + sigWidth, yPos + 20);
    doc.line(margin + sigWidth + 15, yPos + 20, pageWidth - margin, yPos + 20);
    
    doc.setFontSize(7);
    doc.setTextColor(100, 116, 139);
    doc.setFont('helvetica', 'normal');
    doc.text('Prepared By (Name & Signature)', margin + (sigWidth / 2), yPos + 26, { align: 'center' });
    doc.text('Approved By (Name & Signature)', margin + sigWidth + 15 + (sigWidth / 2), yPos + 26, { align: 'center' });
